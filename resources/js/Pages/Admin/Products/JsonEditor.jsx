...
/* ── JSON Visual Page Builder Component ── */
export default function JsonEditor({ value, onChange, errors }) {
    ...
    // Automatically initialize empty or invalid JSON values to a default layout on mount or empty
    useEffect(() => {
        if (!value || !value.trim() || parsedData === null) {
            onChange(JSON.stringify(boardTemplate, null, 2));
        }
    }, [value, parsedData, onChange]);

    const data = parsedData || boardTemplate;

    const layout = (() => {
        if (data.specs_cards || data.overview) return 'machine';
        if (data.requirements || data.debugging_guide || data.peripherals) return 'embcore';
        return 'board';
    })();
    ...
    const handleLayoutChange = (newLayout) => {
        ...
        if (newLayout === 'machine') template = machineTemplate;
        if (newLayout === 'embcore') template = embcoreTemplate;

        if (window.confirm(`Switching to "${newLayout.toUpperCase()}" layout will replace current page sections with the default template. Proceed?`)) {
                            ...
                            {(data.peripherals?.cards || []).map((card, idx) => (
                                <div key={idx} style={cardStyle}>
                                    ...
                                    <div style={{ marginTop: '12px' }}>
                                        ...
                                        <label style={fieldLabelStyle}>Card Specs/Items (One per line)</label>
                                        <textarea
                                            value={card.items?.join('\n') || card.specs?.join('\n') || ''}
                                            onChange={(e) => {
                                                const lines = e.target.value.split('\n').filter(Boolean);
